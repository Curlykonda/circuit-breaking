...
from inference import criterion, infer_batch_with_owt, prepare_fixed_demo
    ...
    for e in tqdm(range(epochs_left)):
        for c, batch in enumerate(toxic_data_loader):
            ...
            for p in mask_params:
                ...
                penalty += max(
                    0, p.sum() * (epochs_trained - 20) / 10000
                )  # why 2000? free
            ...
            tox_loss, owt_loss = infer_batch_with_owt(
                model, criterion, batch, next(owt_iter)["tokens"], batch_size, demos
            )
...
# px.line(y=losses, x=np.arange(len(losses))*(model.cfg.n_ctx * batch_size),
# labels={"y":"Loss", "x":"Tokens"}, title="Training curve for my tiny demo model!")
