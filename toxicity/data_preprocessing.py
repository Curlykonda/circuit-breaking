...
with open(
    "/home/ubuntu/circuitbreakers/data/4chan/pol_062016-112019_labeled.ndjson"
) as f:
    # verbose
    print("Loading data...")
...
# `get_data` function constructs a sample of size `num_samples`, with toxicity
# scores unfiromly disrtibuted over the `toxicity_range` argument
def get_data(
    toxicity_scores,
    num_samples,
    toxicity_range=[0, 1],
    ...
    bucket_ranges = toxicity_range[0] + torch.linspace(0, 1, num_buckets + 1) * (
        toxicity_range[1] - toxicity_range[0]
    )
    ...
    for i in range(len(bucket_indices) - 1):
        ...
        samples = random.sample(
            toxicity_scores[start:end], samples_per_bucket + samples_per_bucket_test
        )
...
data, _ = get_data(toxicity_scores, 1000, toxicity_range=[0.9, 1])
plt.hist([x[1] for x in data], bins=100)
plt.title("Distribution of Toxicity Scores")


train_set, test_set = get_data(
    toxicity_scores, 1000, toxicity_range=[0.9, 1], num_test_samples=2000
)
