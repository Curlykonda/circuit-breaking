...
from data import (
    ...
    retrieve_owt_data,
    retrieve_toxic_data,
    retrieve_toxic_data_low_loss,
    retrieve_toxic_filtered_data,
)
from detoxify import Detoxify
from inference import criterion, generate_from_tokens, infer_batch, prepare_fixed_demo
...
def evaluate_toxic(model, toxic_data_loader, toxic_batches=5, demos=False):
    batch_demos = prepare_fixed_demo(
        tokenizer, toxic_data_loader.batch_size, demo=""
    ).to(DEVICE)
    losses = []
    toxicities = []
    for c, batch in enumerate(toxic_data_loader):
        ...
        with torch.no_grad():
            ...
            losses.append(
                infer_batch(
                    model, criterion, batch, toxic_data_loader.batch_size, batch_demos
                ).item()
            )
            completions = generate_from_tokens(
                model,
                batch_demos if demos else batch,
                max_length=30,
                temperature=0,
                return_new_only=True,
            )
        string_completions = tokenizer.batch_decode(
            completions, skip_special_tokens=True
        )
        toxicities.append(
            np.mean([toxic_scorer.predict(x)["toxicity"] for x in string_completions])
        )
...
def evaluate_owt(model, owt_data_loader, demos=False, owt_batches=10):
    ...
    if demos is False:
        batch_demos = prepare_fixed_demo(
            tokenizer, owt_data_loader.batch_size, demo=""
        ).to(DEVICE)
    else:
        demos = cycle(demos)
    for c, batch in enumerate(owt_data_loader):
        ...
        with torch.no_grad():
            losses.append(
                infer_batch(
                    model, criterion, batch, owt_data_loader.batch_size, batch_demos
                ).item()
            )
...
def evaluate_model(
    model, owt_data_loader=None, toxic_samples=100, toxic_batches=5, owt_batches=10
):
    model.eval()
    ...
    toxic_filtered_data_loader = retrieve_toxic_filtered_data(
        toxic_samples, split="test"
    )
    evaluate_toxic(
        model, toxic_filtered_data_loader, toxic_batches=toxic_batches, demos=True
    )
...
if __name__ == "__main__":
    from models import import_finetuned_model, model
