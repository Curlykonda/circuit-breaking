...
from data import (
    CONTEXT_LENGTH,
    FILTER_DEMO_LEN,
    FILTER_GENERATED_LEN,
    tokenize_and_concatenate_list,
)
...
def filter_toxic_data(
    ...
    file_name="train",
    threshold=0.9,
    demo_len=FILTER_DEMO_LEN,
    max_length=FILTER_GENERATED_LEN,
    toxicity_batch_size=100,
):
    model.eval()
    with torch.no_grad():
        ...
        for i in range(batches):
            ...
            toxic_batch = tokenize_and_concatenate_list(
                toxic_prompts[start_batch:end_batch], tokenizer, demo_len
            ).to(DEVICE)
            token_completions = generate_from_tokens(
                model,
                toxic_batch,
            ...
            string_completions = tokenizer.batch_decode(
                token_completions, skip_special_tokens=True
            )
            ...
            for j, score in enumerate(tqdm(toxic_scores)):
                if score > threshold:
                    indices.append(j)
            filtered_prompts = torch.cat(
                (filtered_prompts, toxic_batch[indices]), dim=0
            )
            filtered_completions = torch.cat(
                (filtered_completions, token_completions[indices]), dim=0
            )
            if filtered_prompts.shape[0] >= samples:
                ...
                return
        print(
            f"NOTE: only produced {filtered_prompts.shape[0]} toxic-completion samples"
        )
...
def filter_toxic_data_low_loss(
    model,
    tokenizer,
    toxic_prompts,
    samples=100,
    ...
    with torch.no_grad():
        ...
        for i in range(batches):
            ...
            filtered_prompts = torch.cat(
                (filtered_prompts, toxic_batch[seq_loss < threshold]), dim=0
            )
...
filter_toxic_data(
    model, tokenizer, [t[2] for t in test_set], samples=500, file_name="test"
)
# %%
