...
import torch
from data import CONTEXT_LENGTH, retrieve_owt_data, retrieve_toxic_data
...
train_loader_toxic = retrieve_toxic_data(
...
test_loader_toxic = retrieve_toxic_data(
    batch_size=BATCH_SIZE // 2,
    ctx_length=CONTEXT_LENGTH,
    tokenizer=tokenizer,
    split="test",
...
print(
    "Number of samples in test_loader_toxic:", len(test_loader_toxic) * BATCH_SIZE // 2
)
...
def initialize(
    ...
    init_toxic_train_losses, init_owt_train_losses, init_joint_train_losses = joint_inf(
        model, None, train_loader_toxic, train_owt_cycle, RATIO, DEVICE
    )
    init_toxic_test_losses, init_owt_test_losses, init_joint_test_losses = joint_inf(
        model, None, test_loader_toxic, test_owt_cycle, RATIO, DEVICE
    )
...
for epoch in range(NUM_EPOCHS):
    toxic_losses, owt_losses, joint_losses = joint_inf(
        model, optimizer, train_loader_toxic, train_owt_cycle, RATIO, DEVICE
    )
    ...
    # log
    print(
        f"Epoch {epoch+1} | Toxic Train Loss: {toxic_train_losses[-1]} | Toxic Test Loss: {toxic_test_losses[-1]}"
    )
    print(
        f"Epoch {epoch+1} | OWT Train Loss: {owt_train_losses[-1]} | OWT Test Loss: {owt_test_losses[-1]}"
    )
...
plt.plot(
    [i * len(train_loader_toxic) for i in range(len(toxic_test_losses))],
    toxic_test_losses,
    label="Toxic Test Loss",
)

plt.plot(owt_train_losses, label="OWT Train Loss")
plt.plot(
    [i * len(train_loader_toxic) for i in range(len(owt_test_losses))],
    owt_test_losses,
    label="OWT Test Loss",
)
