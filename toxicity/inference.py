...
def infer_batch(
    model, criterion, batch, batch_size, demos, device=DEVICE, itemized=False
):
    ...
    return evaluate_sequence_loss(
        out, input, criterion, demos.shape[1], itemized=itemized
    )


def infer_batch_with_owt(
    model, criterion, toxic_batch, owt_batch, batch_size, demos, device="cuda"
):
    # encode the batch
    ...
    return evaluate_sequence_loss(
        out[: toxic_batch.shape[0]],
        input[: toxic_batch.shape[0]],
        criterion,
        demos.shape[1],
    ), evaluate_sequence_loss(
        out[toxic_batch.shape[0] :],
        batch[toxic_batch.shape[0] :],
        criterion,
        demos.shape[1],
    )
...
def generate_text(model, tokenizer, prompt, max_length=20, temperature=0):
    ...
    output = model.generate(
        input_ids, temperature=temperature, max_new_tokens=max_length
    )
    ...
    for _ in tqdm(range(max_length)):
        ...
        if attention_mask is not None:
            attention_mask = torch.cat(
                [attention_mask, torch.ones((attention_mask.shape[0], 1)).to(DEVICE)],
                dim=-1,
            )
    if return_new_only:
...
# batched
def generate_no_hf(
    model, tokenizer, prompts, max_length=50, temperature=0, return_new_only=True
):
...
def generate_no_hf_new(
    ...
    for prompt in tqdm(prompts):
        ...
        if return_new_only:
            outputs.append(
                tokenizer.decode(prompt[orig_len:], skip_special_tokens=True)
            )
        else:
