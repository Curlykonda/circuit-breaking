...
def import_finetuned_model(mode="finetuned", ratio=RATIO):
    ...
    if mode == "finetuned":
        model_ft.load_state_dict(
            torch.load(f"models/joint_inf_ratio={ratio}_max_20.pt")
        )
    elif mode == "ascent":
        model_ft.load_state_dict(
            torch.load("models/ascent_non_toxic_model_best_50_epochs.pt")
        )
    elif mode == "algebra":
        model_ft.load_state_dict(torch.load("models/task_algebra_non_toxic_model.pt"))
    else:
...
def import_ablated_model(version, means):
    with open(
        f"models/masked_gpt2_mean_ablation_v{version}.pkl", "rb"
    ) as f:  # "masked_gpt2.pkl", "rb") as f:
